...
```{r 4, comment=NA}
#generamos una tabla para cada variable no numérica
frecuenciaVecinador = table(datos$Vecindario)
frecuenciaNombre = table(datos$Nombre_propietario)
#Para la tabla de nombre quitamos todos aquellos que no superan las 20 apariciones para no sobrecargar la tabla con muchos nombres
...
#mostramos por pantalla las tablas
frecuenciaVecinador
frecuenciaNombre
frecuenciaTipo
```

## **5. Boxplots y histogramas de las variables numericas.**

Boxplots de las variables numericas. 


```{r fig.width=10, fig.height= 10, echo= FALSE}
#Ponemos para que los gráficos se puedan mostrar a dos por línea y ocupen menos espacio
par(mfrow=c(2,2))


boxplot(datos$Lat., main = "Boxplot de la variable Latitud", col = "blue", ylab = "Latitud")

boxplot(datos$Long., main = "Boxplot de la variable Longitud", col = "pink", ylab = "Longitud")

boxplot(datos$Precio, main = "Boxplot de la variable Precio", col = "red", ylab = "Precio")

boxplot(datos$Min.noches, ylim = c(1,100), main = "Boxplot de la variable Mínimo de Noches", col = "green", ylab = "Mínimo de noches")

boxplot(datos$N_reseñas, main = "Boxplot de la variable Numeros de reseñas", col = "orange", ylab = "Numero de reseñas")

boxplot(datos$Reseñas_mes, main = "Boxplot de la variable Numeros de Reseñas por mes" , col = "yellow", ylab = "Numero de reseñas por mes")

boxplot(datos$Disponibilidad_año , main = "Boxplot de la variable Disponibilidad por año", col = "purple", ylab = "Disponibilidad por año")
...
hist(datos$Precio, main= "Histograma de la variable Precio", xlim = c(7,600),xlab = "Precio", ylab= "Frequencia", col = "red")
...
#Resetamos el "par" para evitar tener problemas
par(mfrow=c(1,1))
```

## **6. Pie chart de variables no numéricas.**
```{r 6, echo = FALSE, comment=NA}
...
frecuenciaVecindario <- frecuenciaVecindario[frecuenciaVecindario>0.02*habitacionesTotal] #Excluimos las ciudades que no representan ni un 2% del total para no sobrecargar el gráfico pues si no lo hacemos no lo podremos leer
...
## **8. Boxplot de la variable precio, para los 5 barrios más caros y los 5 barrios más baratos.**
...
```{r 10, echo = FALSE, comment=NA}
#Ponemos el echo a FALSE porque sino generaremos muchas páginas de texto
...
```{r 11, echo = FALSE, comment = NA}
table(cut(x = datos$Precio, breaks = 5))
table(cut(x = datos$N_reseñas, breaks = 5))
```

## **12. A partir de los datos en intervalos obtenidos en el apartado anterior construid una tabla de contingencia de las dos variables y dibuja el diagrama de mosaico asociado a la mesa.**

```{r 12, echo = FALSE, comment=NA}
#Ponemos el echo a FALSE porque sino generaremos muchas páginas de texto al generar cuts
cuts <- table(Cut1,Cut2)
cuts
par(mar=c(2,1,1,2))
colores <- c("lightblue","lightgreen","purple")
mosaicplot(cuts, main= "Tabla contingencia Precios y Reseñas", col = colores)
```
